...
Foundry eases the whold process since it is written in Solidity. This means our deployment scripts will also be written in Solidity. It is essential to distinguish Solidity as a contract language from Solidity as a scripting language. Foundry also incorporates elements that enhance our Solidity experience beyond the smart contracts realm. So, let's get started on creating a script to deploy our simple storage contract.
...
For more best practice info regarding Foundry scripts click [here](https://getfoundry.sh/guides/best-practices/writing-contracts/#scripts).
...
**NOTE:** `forge-std` also called Forge Standard Libray is a collection of pre-written Solidity contracts designed to simplify and enhance scripting and testing within the Foundry development framework.
...
Please select the `Avil` terminal and press `CTRL(CMD) + C` to stop it. Now run the following command:
...
Answer: No, the output indicates this was a simulaton. But, we got a new folder out of this, the `broadcast` folder contains information about different script runs in case we forget details.
